...
import { ArrowRight, ChevronRight, Shield, Swords, Snowflake, Wind, Zap, Droplets, Mountain, Sparkles, Flame, BookOpen, Crown, Users } from 'lucide-react';
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
      ...
      {/* ===== HERO TIER TABLE (Data-Driven Table 1) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            return (
              <Link key={hero.id} href="/tools-tier-list" className="grid grid-cols-[2rem_1fr_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr_1fr] items-center gap-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                ...
                <span className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">{translate(t, hero.nameKey) || hero.id}</span>
                </span>
      ...
      {/* ===== STATUS AILMENTS CARDS (Data-Driven Cards 1) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              return (
                <Link key={ailment.id} href="/status-ailments" className="block rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm hover:border-[var(--color-accent)]/30 transition-all duration-300 group">
                  ...
                  <h3 className="font-bold mb-1 group-hover:text-[var(--color-accent)] transition-colors">
                    {translate(t, ailment.nameKey) || ailment.id}
                  </h3>
                  <p className="text-sm text-[var(--color-text-secondary)]">{translate(t, ailment.descKey)}</p>
                </Link>
              );
            })}
          </div>
      ...
      {/* ===== FAMILIAR TABLE (Data-Driven Table 2) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
        ...
        <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
          <div className="grid grid-cols-[2rem_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr] gap-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
            <span></span><span>Familiar</span><span>Source</span><span>Type</span>
          </div>
          {familiars.map((familiar, idx) => {
            const badgeVal = translate(t, familiar.badgeKey);
            const badgeColor = familiar.badgeKey.includes('bonus') || familiar.badgeKey.includes('dlc')
              ? 'var(--color-accent)'
              : 'var(--color-tier-a)';
            return (
              <Link key={familiar.id} href="/familiars" className="grid grid-cols-[2rem_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr] items-center gap-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                ...
                <span><span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: badgeColor, background: `${badgeColor}15` }}>{badgeVal}</span></span>
              </Link>
            );
          })}
        </div>
      ...
      {/* ===== TIER LIST CARDS (Data-Driven Cards 2) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
            ...
            {['S', 'A', 'B', 'C'].map((tier) => {
              ...
              return (
                <Link key={tier} href="/tier-list" className="block rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm hover:border-[var(--color-accent)]/30 transition-all duration-300 group">
                  ...
                  <h3 className="font-bold mb-2 group-hover:text-[var(--color-accent)] transition-colors">
                    {tier === 'S' ? 'Dominant' : tier === 'A' ? 'Strong' : tier === 'B' ? 'Viable' : 'Niche'}
                  </h3>
                  <p className="text-sm text-[var(--color-text-secondary)]">
                    {tierHeroes.slice(0, 4).map(h => translate(t, h.nameKey) || h.id).join(', ')}{tierHeroes.length > 4 ? ` +${tierHeroes.length - 4} more` : ''}
                  </p>
                </Link>
              );
            })}
