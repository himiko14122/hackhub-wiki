...
Heavy graphical scanners look flashy, but they cost precious seconds when the mission clock is already red. Nslookup shines because it returns a single record family per call, so you can chain queries without waiting for a full port sweep to finish. If you want a primer on how recon chains together inside the game, the [HackHub tools overview](/en/tools/) covers the broader toolkit and shows where nslookup slots into a typical run.
